# My Professional Development Journey 

## Current Goal
Applying for a Technology & Transformation Summer Internship at JP Morgan

##  Key Progress
- **SQL Skills:** Completed S&P 500 ETL pipeline.
- **Financial Literacy:** Learning 1 KPI per week.
- **Industry Awareness:** Following FT & Deloitte Ireland Insights.  

## Repository map

* **[Finance Dictionary](./finance-dictionary/)**:
  Definitions for key financial metrics.

* **[Industry Insights](./industry-insights/)**:
  Targeted research on market trends and sectors aligned with Deloitte’s industry perspectives.

* **[LeetCode Log](./leetcode-log/)**:
  Daily challenges, including problems and my solutions.

* **[Technical Log](./technical-log/)**:
  Notes on functions, debugging, and data logic.

  ## External Profiles

* **LeetCode Profile:** [View my daily streak](https://leetcode.com/u/eesha3/)
* **LinkedIn:** [Connect with me](https://www.linkedin.com/in/eesha-penkey-681a123bb/)
